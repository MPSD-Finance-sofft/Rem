module Reports::LeasingContracts
  ...
  def sum_purchace_price_for_period(from, to)
    Accord.where(state: 'contract').where("date_of_signature <= ? AND date_of_signature >= ?", to,from).sum(:purchase_price)
  end

  def count_purchace_price_for_period(from, to)
    Accord.where(state: 'contract').where("date_of_signature <= ? AND date_of_signature >= ?", to,from).count
  end

  def count_leasing_contract_from_period(from, to )
    LeasingContract.where("rent_from <= ? AND rent_from >= ?",to, from).count
  end

  def sum_purchace_price_leasing_contract_from_period(from, to)
    LeasingContract.where("rent_from <= ? AND rent_from >= ?",to, from).joins(:accord).sum(:purchase_price)
  end

  def sum_monthly_rent_leasing_contract_from_period(from, to)
    LeasingContract.where("rent_from <= ? AND rent_from >= ?",to, from).sum(:monthly_rent)
  end 
