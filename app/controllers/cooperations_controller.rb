class CooperationsController < ApplicationController
  ...
  def index
    ...
    @agents_with_cooperation = User.agents.includes(:cooperations).order(date_of_cooperation: :desc).where("date_of_cooperation <= ?", "1.9.2019".to_date).select{|a| a.can_sign_in}
  end
