class SalesContractsController < ApplicationController
  ...
  def index
    ...
    @sales_contracts = SalesContract.order(date_of_receipt_of_payment: :desc).decorate
  end
  ...
  def report
    authorize SalesContract
    @sales_contracts = SalesContract.includes(accord: :expenses).includes(accord: :leasing_contracts).order(date_of_sale_realty: :desc).includes(:realty).decorate
