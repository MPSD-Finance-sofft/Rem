  ...
  private
  	def chart
        ...
        accords = Accord.where(id: policy_scope(Accord).pluck(:id))
    		...
    		contract_data =  accords.where(state: 'contract').select("accords.created_at, month(date_of_signature) as month, year(date_of_signature) as year, count(id) as count, accords.user_id, accords.agent_id").group(:month,:year).map{|a| [Date.new(a.year, a.month,1), a.count]}
